...
public class UserController {
    ...
    @Autowired
    public UserController(OrderRepository orderRepository, CartRepository cartRepository, ProductService productService, TortFillingRepository tortFillingRepository, ProductRepository productRepository) {
...
//        System.out.println("Пароль пользователя: " + personDetails.getPerson().getPassword());
    ...
    public String addProductInCart(@PathVariable("id") int id, Model model){
        ...
        Cart cart = new Cart(id_person, product.getId(),tortFillingRepository.findByName("EMPTY"));
    ...
    public String cart(Model model){
        ...
        List<Object> storageList = new ArrayList<>();
        float price = 0;
        for (Cart cart: cartList) {
            productList.add(productService.getProductId(cart.getProductId()));
            List<Object> storage = new ArrayList<>();
            storage.add(cart); //корзина с товаром
            storage.add(productService.getProductId(cart.getProductId()));//сам товар
            storageList.add(storage);
            price += productService.getProductId(cart.getProductId()).getPrice()*cart.getQty();
...
//    }
    ...
    public String createOrder(){
        ...
        //Делаем создание заказа из корзины с добавлением новых полей
        String uuid = UUID.randomUUID().toString();
        Product product = new Product();
        for (Cart cart: cartList) {
            //productList.add(productService.getProductId(cart.getProductId()));
            product = productService.getProductId(cart.getProductId());
            Orders newOrder = new Orders(uuid, cart.getQty(), product.getPrice(), Status.Оформлен, product, personDetails.getPerson(), cart.getComment(), cart.getDate(), cart.getTortFilling());
            orderRepository.save(newOrder);
            cartRepository.deleteCartById(product.getId(), id_person);
        }
    ...
    public String ordersUser(Model model){
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        PersonDetails personDetails = (PersonDetails) authentication.getPrincipal();
        List<Orders> orderList = orderRepository.findByPerson(personDetails.getPerson());
        float summ = 0;
    ...
    // Метод по получению записи корзины по по id и отображение шаблона редактирования
    @GetMapping("/cart/editCart/{id}")
    public String editCart(@PathVariable("id") int id, Model model){
        model.addAttribute("editCart", cartRepository.findById(id));
        model.addAttribute("tortFilling", tortFillingRepository.findAll());
        return "/user/editCart";
    }

   @PostMapping("/cart/editCart/{id}")
    public String editCart(@ModelAttribute("editCart") Cart cart, @PathVariable("id") int id){
    ...
    public String indexSearch(@RequestParam("search") String search, @RequestParam("ot") String ot, @RequestParam("do") String Do, @RequestParam(value = "price", required = false, defaultValue = "") String price, @RequestParam(value = "category", required = false, defaultValue = "") String category, Model model){
            ...
            if (!price.isEmpty()) {
                ...
                if (price.equals("sorted_by_ascending_price")) {
                    ...
                    if (!category.isEmpty()) {
                        ...
                        } else if (category.equals("corporative")) {
        ...
        } else { //без диапазона цен или если не указана одна из границ диапазона
